...
import { fetchRemoteCost, createQuoteRequestData, ensureFormDefaults, transformRemoteResponseToQuote } from "@/lib/shared/utils/apiUtils";
...
export const useRemoteQuote = () => {
  ...
  const calculateRemoteQuote = useCallback(async (formData: EORFormData, data: QuoteData): Promise<QuoteData> => {
    ...
    try {
      ...
      const remoteQuote = transformRemoteResponseToQuote(remoteQuoteResponse);
      ...
      const dualCurrencyQuotes = localRemoteQuoteResponse ? {
        ...
        remote: {
          selectedCurrencyQuote: remoteQuote,
          localCurrencyQuote: transformRemoteResponseToQuote(localRemoteQuoteResponse),
